...
if(!empty($instance['zoomable']) ? $zoomable = 'data-action="zoom"' : $zoomable = '');
...
<?php if(!empty($instance['url'])) : ?><a href="<?php echo sow_esc_url($instance['url']) ?>" <?php if($instance['new_window']) echo 'target="_blank"' ?>><?php endif; ?>
	<img <?php foreach($attr as $n => $v) echo $n.'="' . esc_attr($v) . '" ' ?> class="<?php echo esc_attr( implode(' ', $classes) ) ?>" <?php if( !empty($styles) ) echo 'style="'.implode('; ', $styles).'"'; ?>  <?=$zoomable?>/>
